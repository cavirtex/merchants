...
function getProvinceCode($province){
  ...
  class cavirtex {
    ...
    function after_process() { 
		global $_POST, $_GET, $insert_id, $db, $currencies;
		...
		if (!$_GET['zenid']) {
			$totalsum = $order->info['total'] = zen_round($order->info['total'], 2);
			
			//$order->info['currency'] => CAD
			//$totalInPaymentCurrency=$order->info['total'];
			$totalInPaymentCurrency=zen_round($totalsum * $currencies->get_value("CAD"), 2);
			...
			if(isset($invoice['error'])) {
				zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message='.$invoice['error'], 'SSL', true, false));
				return false;
			} else{
				if (isset($invoice["order_key"]))
				{
					...
					zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=No Order Key. '.$errormsg, 'SSL', true, false));
					return false;
				}
			}
			
			$db->Execute("update " . TABLE_ORDERS . " set cavirtex_order_key = '".$order_key."' where orders_id = ".$insert_id );
			
			zen_redirect($url);
		} else {
			if ($_GET['ec']!='0'){
				zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message='.$_GET['em'], 'SSL', true, false));
			}
		}
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_CAVIRTEX_STATUS'");
    ...
    function install() {
	  ...
	  if (defined('MODULE_PAYMENT_CAVIRTEX_STATUS')) {
	  	...
	  	zen_redirect(zen_href_link(FILENAME_MODULES, 'set=payment&module=cavirtex', 'NONSSL'));
	  	return 'failed';
	  }
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('PCaVirtEx Merchant Key', 'MODULE_PAYMENT_CAVIRTEX_MERCHANT_KEY', 'merchant_key', 'Your Merchant Key, supplied by CaVirtEx', '6', '0', now())");
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('CaVirtEx Merchant Secret Key', 'MODULE_PAYMENT_CAVIRTEX_MERCHANT_SECRET_KEY', 'secret_key', 'Your Secret Key, supplied by CaVirtEx', '6', '1', now())");
  	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Order Status', 'MODULE_PAYMENT_CAVIRTEX_STATUS_ID', '0', 'Set the status of orders made with this payment module (but no payment confirmed) to this value', '6', '2', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
  	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Order Status', 'MODULE_PAYMENT_CAVIRTEX_CONFIRMED_STATUS_ID', '0', 'Set the status of orders made with this payment module (when payment is confirmed) to this value', '6', '2', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
  	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_CAVIRTEX_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '3', now())");
	  ...
	  $check_ok_query = $db->Execute("show columns from " . TABLE_ORDERS . " LIKE 'cavirtex_order_key'");
	  if ($check_ok_query->RecordCount() < 1) {
		  $db->Execute("alter table " . TABLE_ORDERS . " add column cavirtex_order_key varchar(64) after payment_method");
	  }	  
   }
