...
function virtexVerifyNotification($apiKey, $orderKey) {

	$_url="https://www.cavirtex.com/merchant_invoice_balance_check?merchant_key=".$apiKey."&order_key=".$orderKey;
	
	
	$response = virtexCurl2($_url);
	if (is_string($response))
		return array('error' => $response);
	return $response;
}
